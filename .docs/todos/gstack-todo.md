...
- [ ] **键盘快捷键** — 设计决策要求 ⌘K 搜索、⌘⌫ 删除选中项、Enter 确认、Esc 取消，均未实现
- [ ] **侧边栏折叠** — 设计决策要求窗口 < 1100px 自动收起为 64px 图标模式，未实现
- [ ] **错误 toast 通知** — 扫描/清理部分失败时应有 toast 提示，当前静默处理
- [ ] **暗色模式** — theme.css 已有完整暗色变量，v1 决定只做浅色，v2 需实现切换逻辑
...
| 响应式/无障碍 | 6/10 | 侧边栏折叠未实现，键盘导航未实现，ARIA 标签缺失 |
